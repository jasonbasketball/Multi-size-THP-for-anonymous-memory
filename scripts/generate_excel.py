#!/usr/bin/env python3
"""
Generate Excel file with Multi-size THP integration information
"""
...
def create_excel_report():
    ...
    overview_data = [
        ["", "", "", ""],
        ["项目名称", "Multi-size THP for Anonymous Memory", "", ""],
        ["功能描述", "多尺寸透明大页为匿名内存提供运行时选择功能", "", ""],
        ["源内核版本", "Linux 6.8", "", ""],
        ["目标内核版本", "OpenEuler 6.6", "", ""],
        ["主要贡献者", "Ryan Roberts (ARM)", "", ""],
        ["特性状态", "已合入主线 (Mainline Merged)", "", ""],
        ["生成日期", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "", ""],
        ["", "", "", ""],
        ["功能特点", "", "", ""],
        ["1", "支持多种大小的透明大页 (64KB, 128KB, 256KB, 512KB, 1MB, 2MB)", "", ""],
        ["2", "通过sysfs接口进行运行时配置", "", ""],
        ["3", "提供每种大小的分配统计信息", "", ""],
        ["4", "支持ARM64架构的连续页表项优化", "", ""],
        ["5", "改进页错误处理性能", "", ""],
    ...
    for row_idx, row_data in enumerate(overview_data, start=2):
    ...
    for row in range(3, 9):
        ...
        ws1.cell(row=row, column=1).fill = PatternFill(start_color="E7E6E6", end_color="E7E6E6", fill_type="solid")
    ...
    # Load commit data
    try:
        with open('mthp_commits_info.json', 'r', encoding='utf-8') as f:
    ...
    for row_idx, row_data in enumerate(stats_data, start=1):
        for col_idx, value in enumerate(row_data, start=1):
    ...
    for row_idx, row_data in enumerate(plan_data, start=1):
        for col_idx, value in enumerate(row_data, start=1):
